/* Driver for the data translation DT330 series cards. For the moment,
   only th e DT335 digital io is supported. Desciption see below.
...
static unsigned long intcounter[8]; /* tick counters for each interrupt */
...
static void dt330_bh_handler(void *unused) { 
    last_IRQ_note |= (pending & notify_flag);
    /* tell it to the interested.. */
    kill_fasync(&my_async_queue, SIGIO, POLL_IN);
    return;
}
struct tq_struct notify_task = { routine: &dt330_bh_handler,
				 data: NULL,
				 sync: 0,
};

static void dt330_int_handler(int irq, void *dev_id, struct pt_regs *regs) {
    /* arguments are not needed here */
    static unsigned long pm;
    static int i;
    /* get interrupt status register */
    pending = readl(cardspace+DT330_STATUS_REG);
    mb();
    ...
    writel(pending,cardspace+DT330_STATUS_REG);
...
static int dt330_flat_open_irq(struct inode *inode, struct file *filp) {
	...
	return -EBUSY;
    /* get irq */
    retval=request_irq(irq_number, dt330_int_handler, 
...
static int dt330_flat_close_irq(struct inode *inode, struct file *filp) {
    ...
    intstat=readl(cardspace+DT330_CONTROL_0_ADR) & ~0x0ff0; /* DIO IRQs */
    mb();writel(intstat,cardspace+DT330_CONTROL_0_ADR);
...
static int dt330_flat_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg) {
    int dir = _IOC_DIR(cmd);
    unsigned long adr = cmd & IOCTL_ADRMASK;
/* printk("dt330: ioctl attemt to dev 0, cmd: %x  vaule: %x \n",cmd, arg); */
	...
	if (((cmd & 0x3fffffe0) == 0x10000) && ((cmd & 0x1f) < 11)) {
		...
		default: /* accss to the irq event counters */ 
		    return rw_regs(&intcounter[cmd & 0x7],arg, dir);
	    }
	}
...
static struct pci_device_id dt330_pci_tbl[] = __initdata {
    {PCI_VENDOR_ID_DATX, PCI_DEVICE_ID_DT335, PCI_ANY_ID, PCI_ANY_ID, },
    {0,},
};
...
static int __init dt330_init(void) {
    int rc;
    rc = pci_module_init( &dt330_driver );
